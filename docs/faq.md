...
Ollama pulls models from the Internet and may require a proxy server to access the models. Configuring the server with environment variable `HTTPS_PROXY` should be sufficient to redirect outbound requests through the proxy. When using `HTTPS_PROXY`, ensure the proxy certificate is installed as a system certificate. Refer to the section above for how to use environment variables on your platform.
